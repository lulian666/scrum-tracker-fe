...
export const updateBoard = createAsyncThunk(
  ...
  async (newData, { dispatch, getState }) => {
    ...
    const response = await axios.patch(`/api/v1/boards/${board.id}`, newData);
...
export const reorderList = createAsyncThunk(
  ...
  async (result, { dispatch, getState }) => {
    ...
    const response = await axios.patch(`/api/v1/boards/${board.id}`, {
      lists: ordered,
    });
...
export const deleteBoard = createAsyncThunk(
  ...
  async (params, { dispatch, getState }) => {
    ...
    const response = await axios.delete(`/api/v1/boards/${board.id}`);
